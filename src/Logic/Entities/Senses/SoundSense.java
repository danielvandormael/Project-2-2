    ...
    private void canHear(Entity[] guards, Entity[] intruders){
        ...
        for(int i = 0; i<guards.length; i++){
            if(entity.getId() != guards[i].getId()){
                if(guards[i].soundSense.isWalkSound() == true && entity.distanceBetween(guards[i].movement.getX(), guards[i].movement.getY()) <= hearRadius){
                    double[] dirVector = entity.directionToCoords(guards[i].movement.getX(), guards[i].movement.getY());
                    sounds.add(new Sound(dirVector[0], dirVector[1], false));
                }
                if( guards[i].soundSense.isShoutSound() == true && entity.distanceBetween(guards[i].movement.getX(), guards[i].movement.getY()) <= hearRadius){
                    double[] dirVector = entity.directionToCoords(guards[i].movement.getX(), guards[i].movement.getY());
        ...
        for(int i = 0; i<intruders.length; i++){
            if(intruders[i] != null){
                if(entity.getId() != intruders[i].getId()){
                    if(intruders[i].soundSense.isWalkSound() == true && entity.distanceBetween(intruders[i].movement.getX(), intruders[i].movement.getY()) <= hearRadius){
                        double[] dirVector = entity.directionToCoords(intruders[i].movement.getX(), intruders[i].movement.getY());
                        sounds.add(new Sound(dirVector[0], dirVector[1], false));
                    }
                }
            }
