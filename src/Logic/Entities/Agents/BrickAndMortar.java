...
public class BrickAndMortar extends Guard {
    ...
    ArrayList<NodeDFS> accessible = new ArrayList<>();
    ...
    public void BAM(){
        if (((int) movement.getX() == desiredX && (int) movement.getY() == desiredY && vision.getViewAngle() == desiredAngle)
                || ((int) movement.getX() > (desiredX + 1) || (int) movement.getX() < (desiredX - 1)
                || (int) movement.getY() > (desiredY + 1) || (int) movement.getY() < (desiredY - 1))) {
            Marking();
            Navigation();
        }
        else{
    ...
    public void move(){
        NodeDFS currentNode = this.map.getNode(this.movement.getX(), this.movement.getY());

        // The B&M Algorithm only works individually, two B&M entities behave identically
        if(map.isInDirection(currentNode, this.targetNode, this.vision.getViewAngle())) {
            // move towards target
            this.decision[0] = 1;
            this.decision[1] = 0;
            this.moved = true;
